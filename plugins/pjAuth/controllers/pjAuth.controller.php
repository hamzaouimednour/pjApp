...
class pjAuth extends pjAuthAppController
{
    ...
    {
        return pjAuthRoleModel::factory()->where('is_backend', 'T')->orderBy('t1.id ASC')->findAll()->getData();
    }
    ...
    {
        if (!pjAuth::factory('pjBaseUsers', 'pjActionStatusUser')->hasAccess())
        {
            return array('status' => 'ERR', 'code' => 100, 'text' => 'Access denied.');
        }
        if (!pjAuth::factory('pjBaseUsers', 'pjActionProfile')->hasAccess())
        {
            return array('status' => 'ERR', 'code' => 102, 'text' => 'Access denied.');
        }
