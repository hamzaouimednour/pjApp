...
class pjBaseUsers extends pjBase
{
	...
	public function pjActionGetUser()
	{
		...
		if ($this->isXHR())
		{
		    $pjAuthUserModel = pjAuthUserModel::factory()->join('pjAuthRole', 't2.id=t1.role_id', 'left');
		    $pjAuthUserModel->where('t2.is_backend', 'T');
			...
			$data = $pjAuthUserModel
				->select('t1.id, t1.email, t1.name, DATE(t1.created) AS `created`, DATE(t1.last_login) AS `last_login`, t1.status, t1.is_active, t1.locked, t1.role_id, t2.role')
	...
	public function pjActionUpdate()
	{
		...
		$has_revert = pjAuth::factory('pjBaseUsers', 'pjActionStatusUser')->hasAccess();
	...
	public function pjActionSaveUser()
	{
		...
		if ($params['column'] == 'status' && !pjAuth::factory('pjBaseUsers', 'pjActionStatusUser')->hasAccess())
		{
