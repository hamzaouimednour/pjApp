...
class pjBaseSms extends pjBase
{
    ...
    public function pjActionTestSms()
    {
        ...
        if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
        {
            if(!self::isPost())
            {
                self::jsonResponse(array('status' => 'ERR', 'code' => 100, 'title' => __('plugin_base_sms_failed_to_send', true), 'text' => 'HTTP method not allowed.'));
            }
            if(!$this->_post->toString('plugin_sms_api_key'))
            {
                self::jsonResponse(array('status' => 'ERR', 'code' => 101, 'title' => __('plugin_base_sms_failed_to_send', true), 'text' => 'API key is empty.'));
            }
            if(!$this->_post->toString('number'))
            {
                self::jsonResponse(array('status' => 'ERR', 'code' => 102, 'title' => __('plugin_base_sms_failed_to_send', true), 'text' => 'Number is empty.'));
            }
    ...
    public function pjActionVerify()
    {
        ...
        if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
        {
            ...
            if(!$this->_post->toString('plugin_sms_api_key'))
            {
                self::jsonResponse(array('status' => 'ERR', 'code' => 101, 'text' => 'API key is empty.'));
            }
            ...
            {
                self::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => $response['text']));
            }
        }
    }
