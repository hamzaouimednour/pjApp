...
class pjBaseOptions extends pjBase
{
	...
	public function pjActionLoginProtection()
	{
		...
		$this->set('has_access_password', $pjAuth->hasAccess('password'));
		$this->set('has_access_secure_login', $pjAuth->hasAccess('secure_login'));
		$this->set('has_access_failed_login', $pjAuth->hasAccess('failed_login'));
		$this->set('has_access_forgot', $pjAuth->hasAccess('forgot'));
	...
	public function pjActionAjaxSmtp()
	{
		...
		if ($result)
		{
			self::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => 'Connection has been established successfully.'));
		}
		self::jsonResponse(array('status' => 'ERR', 'code' => 102, 'text' => 'Connection fails.', 'message' => $message));
	}
	...
	public function pjActionAjaxSend()
	{
		...
		{
			self::jsonResponse(array('status' => 'ERR', 'code' => 102, 'text' => 'Email address is missing.'));
		} else {
			if (!pjValidation::pjActionEmail($this->_post->toString('email')))
			{
				self::jsonResponse(array('status' => 'ERR', 'code' => 103, 'text' => 'Email address is not valid.'));
			}
		}
		...
		$pjEmail->setSubject(__('plugin_base_test_email_subject', true));
		
		if ($pjEmail->send(__('plugin_base_test_email_message', true)))
		{
			self::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => 'Test email has been sent to ' . $this->_post->toString('email')));
		}
		
		self::jsonResponse(array('status' => 'ERR', 'code' => 104, 'text' => 'Email failed to sent.'));
	}
}
?>