...
class pjBase extends pjBaseAppController
{
	...
	public function __construct($requireLogin=null)
	{
		...
		if ($this->requireLogin && !$this->isInstaller())
		{
		    ...
		    if (!$this->isLoged() && !in_array($_get->toString('action'), array('pjActionLogin', 'pjActionForgot', 'pjActionCaptcha', 'pjActionCheckCaptcha', 'pjActionCheckReCaptcha', 'pjActionCheckLoginEmail', 'pjActionResendPassword', 'pjActionRun', 'pjActionReset')))
			{
				$next = NULL;
	...
	public function pjActionReset()
	{
		...
		if ($this->_get->toString('hash') !== sha1(PJ_SALT . $this->_get->toString('email') . PJ_SALT))
		{
