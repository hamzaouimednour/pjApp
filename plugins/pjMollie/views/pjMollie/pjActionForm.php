...
<script type="text/javascript">
    (function () {
        var form = pjQ.$('#<?php echo $tpl['arr']['id']; ?>');
        if (form) {
            form.validate({
                rules:{
                ...
                submitHandler: function (form) {
                    pjQ.$.post(['<?php echo PJ_INSTALL_URL; ?>', "index.php?controller=pjMollie&action=pjActionGetUrl"].join(""), pjQ.$(form).serialize()).done(function (data) {
                        pjQ.$('#plugin_mollie_payment_form')
            ...
            {
                pjQ.$(document).on("change", 'select[name="mollie_method"]', function (e) {
                    pjQ.$('.vrMollieIDeal').toggle(pjQ.$(this).val() == 'ideal');
                });

                pjQ.$("#modalMollie").on("hidden.bs.modal", function () {
                ...
                pjQ.$('#modalMollie .modal-dialog').css('z-index', 1040);
                pjQ.$('#modalMollie').modal("show");
            }
        }
    })();
</script>
