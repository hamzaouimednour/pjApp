...
class pjAdminTime extends pjAdmin
{
	...
	public function pjActionCheckDefaultTime()
	{
	    ...
	    if(!empty($classes))
	    {
	        self::jsonResponse(array('status' => 'ERR', 'code' => 103, 'classes' => $classes));
	    }else{
	...
	public function pjActionCheckCustomTime()
	{
	    ...
	    if(strtotime($this->_post->toString('start')) >= strtotime($this->_post->toString('end')))
	    {
	        self::jsonResponse(array('status' => 'ERR', 'code' => 103, 'text' => ''));
	    }
	    self::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => ''));
	}
	public function pjActionSaveCustom()
	{
	    ...
	    if ($this->isXHR())
	    {
	        if (self::isPost() && $this->_post->toInt('custom_time') && $this->_post->toInt('location_id'))
	        {
	            ...
	            $pjDateModel->where('type', $this->_post->toString('type'))->where('date', $date)->eraseAll();
